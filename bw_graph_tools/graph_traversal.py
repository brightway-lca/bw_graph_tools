# NOTE: This code was developed by Ben Portner
# It was suggested for contribution to the Activitity Browser:
# https://github.com/LCA-ActivityBrowser/activity-browser/pull/844
# The Activity Browser is LGPL licensed:
# https://github.com/LCA-ActivityBrowser/activity-browser/blob/master/LICENSE.txt
# This code does not have an explicit indication on its license, but is
# heavily based off of the bw2calc Graph Traversal code, which
# is BSD licensed.
    ...
    def add_node_keys(self, rev_act: Dict, rev_bio: Dict) -> None:
        for n in self.nodes:
            if n.index == -1:
                ...
                rev_dict = rev_act if isinstance(n, GTTechnosphereNode) else rev_bio
...
class GTEdgeList:
    ...
    def get_unique_nodes(self) -> Set[GTNode]:
        return set([e.from_node for e in self.edges] + [e.to_node for e in self.edges])
...
class GraphTraversal:
    ...
    def reset(
        self, demand: Dict, method: Tuple
    ) -> List[Tuple[GTTechnosphereNode, Real]]:
        ...
        self.cb = np.array(
            (self.lca.characterization_matrix * self.lca.biosphere_matrix).sum(axis=0)
        ).ravel()
    ...
    def traverse_depth_first(
        ...
        if self.number_calcs >= max_calc:
            print(f"Max. number calculations reached at activity id {to_node.index}")
            return

        scale_value = self.lca.technosphere_matrix[to_node.index, to_node.index]
        ...
        for from_index, from_amount in zip(indices, techno_inputs[indices].data):
    ...
    def traverse_importance_first(
        ...
        while heap:
            ...
            to_scale = self.lca.technosphere_matrix[to_node.index, to_node.index]

            # add biosphere contributions
            if self.include_biosphere:
                bio_inputs = self.lca.biosphere_matrix[:, to_node.index] / to_scale
                indices = bio_inputs.nonzero()[0]
                for from_index, from_amount in zip(indices, bio_inputs[indices].data):
                    from_node = self._get_or_add_biosphere_node(index=from_index)
                    self._add_edge_if_above_cutoff(
